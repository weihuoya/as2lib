 ...
 * @author Simon Wacker
 */
class test.unit.org.as2lib.env.reflect.algorithm.TMethodAlgorithm extends TestCase {
	...
	public function testExecute(Void):Void {
		...
		for (var i:Number = 0; i < c.length; i++) {
			...
			if (k.getName() == "publicMethod") {
				assertSame("publicMethod", k.getMethod(), test.unit.org.as2lib.env.reflect.treflect.SubClass.prototype.publicMethod);
				assertFalse("publicMethod", k.isStatic());
				assertSame("publicMethod", k.getDeclaringType(), p);
			} else if (k.getName() == "privateMethod") {
				assertSame("privateMethod", k.getMethod(), test.unit.org.as2lib.env.reflect.treflect.SubClass.prototype["private" + "Method"]);
				assertFalse("privateMethod", k.isStatic());
				assertSame("privateMethod", k.getDeclaringType(), p);
			} else if (k.getName() == "publicStaticMethod") {
				assertSame("publicStaticMethod", k.getMethod(), test.unit.org.as2lib.env.reflect.treflect.SubClass.publicStaticMethod);
				...
				assertSame("privateStaticMethod", k.getMethod(), test.unit.org.as2lib.env.reflect.treflect.SubClass["private" + "StaticMethod"]);
