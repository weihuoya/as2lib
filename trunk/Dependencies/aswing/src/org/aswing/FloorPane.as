...
class org.aswing.FloorPane extends Container {
    ...
    private var scaleImage:Boolean;
     ...
     * <p><b>Note:</b>Take care to use scaleImage to load a swf, because swf has different size at different frame or
     *  when some symbol invisible/visible.
     * @param b whether need scale the loaded image/animation to fit the pane's size.
     */
     ...
     * @return whether need scale the loaded image/animation to fit the pane's size.
     */
	...
	private function create():Void{
		...
		if(MCUtils.isMovieClipExist(target_mc)){
			floorMCDepth = target_mc.getNextHighestDepth();
	...
	private function fitImage():Void{
		if(isLoaded()){
			...
			if(scaleImage){
				...
				floorMC._width = b.width - offsetX;
				floorMC._height = b.height - offsetY;
				hadscaled = true;
			}else{
				if(hadscaled){
				...
				if(horizontalAlignment == CENTER){
					mx = b.x + (b.width - floorOriginalSize.width)/2;
				}else if(horizontalAlignment == RIGHT){
					mx = b.x + (b.width - floorOriginalSize.width);
				}else{
					mx = b.x;
				}
				if(verticalAlignment == CENTER){
					my = b.y + (b.height - floorOriginalSize.height)/2;
				}else if(verticalAlignment == BOTTOM){
					my = b.y + (b.height - floorOriginalSize.height);
				}else{
					my = b.y;
				}
	...
	private function countPreferredSize():Dimension{
		...
		if(isLoaded()){
			sizeByMC = new Dimension(floorOriginalSize.width - offsetX, floorOriginalSize.height - offsetY);
			sizeByMC = getInsets().roundsSize(sizeByMC);
		}else{
			sizeByMC = super.countPreferredSize();
		}
