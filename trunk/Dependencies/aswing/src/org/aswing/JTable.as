...
class org.aswing.JTable extends ChildrenMaskedContainer implements Viewportable, TableModelListener, 
	...
	private function initWithModels(dm:TableModel, cm:TableColumnModel, sm:ListSelectionModel):Void{
		...
		selectionModelListener[JList.ON_SELECTION_CHANGE] = Delegate.create(this, __listSelectionChanged);
    ...
    public function updateUI():Void {
        ...
        var defaultRenderers:Array = defaultRenderersByColumnClass.values();
        for(var i:Number=0; i<defaultRenderers.length; i++){
            updateSubComponentUI(defaultRenderers[i]);
        }
     ...
     * @see     #getColumnClass()
     */	
	public function getDefaultRenderer(columnClass:String):TableCellFactory{
		...
		if (columnClass == null){
			...
			if (renderer != null){
				...
				return getDefaultRenderer("Object");
			}
		}
	}
	...
	public function getCellRect(row:Number, column:Number, includeSpacing:Boolean):Rectangle{
		...
		if (row < 0){
			...
			r.y = getHeight() - insets.getMarginHeight();
			valid = false;
		}else{
		...
		if (column < 0){
			...
			//TODO 
			//r.x = getWidth() - insets.getMarginWidth();
...
//	}
	...
	/*public function getToolTipText(event:MouseEvent):String{
		var tip:String = null;
		var p:Point = event.getPoint();
		var hitColumnIndex:Number = columnAtPoint(p);
		var hitRowIndex:Number = rowAtPoint(p);
		if ((hitColumnIndex != (- 1)) && (hitRowIndex != (- 1)))
		{
			var renderer:TableCellFactory = getCellRenderer(hitRowIndex, hitColumnIndex);
			var component:Component = prepareRenderer(renderer, hitRowIndex, hitColumnIndex);
			if (component instanceof JComponent)
			{
				var cellRect:Rectangle = getCellRect(hitRowIndex, hitColumnIndex, false);
				p.translate(- cellRect.x, - cellRect.y);
				var newEvent:MouseEvent = new MouseEvent(component, event.getID(), event.getWhen(), event.getModifiers(), p.x, p.y, event.getClickCount(), event.isPopupTrigger());
...
//	public function setSurrendersFocusOnKeystroke(surrendersFocusOnKeystroke:Boolean):Void{
//		this.surrendersFocusOnKeystroke = surrendersFocusOnKeystroke;
//	}
//	public function getSurrendersFocusOnKeystroke():Boolean{
...
//	}
	...
	public function editCellAt(row:Number, column:Number, clickCount:Number):Boolean{
		...
		if ((editor != null) && editor.isCellEditable(clickCount)){
     ...
     */
	public function __listSelectionChanged(fIndex:Number, lIndex:Number):Void{
		var isAdjusting:Boolean = false;//e.getValueIsAdjusting();
	...
	public function getCellRenderer(row:Number, column:Number):TableCellFactory{
		//trace("getCellRenderer...");
		var tableColumn:TableColumn = getColumnModel().getColumn(column);
		//trace("tableColumn = " + tableColumn);
		var renderer:TableCellFactory = tableColumn.getCellRenderer();
	...
	public function prepareRenderer3(renderer:TableCellFactory, row:Number, column:Number):Component{
		var value:Object = getValueAt(row, column);
		...
		return null;//renderer.getTableCellRendererComponent(this, value, isSelected, hasFocus, row, column);
	}
...
//		editorRemover = null;
	...
	private function layoutCells():Void{
		...
		var insetsX:Number = insets.left;
		var insetsY:Number = insets.top;
		var startX:Number = insetsX - viewPosition.x;
		...
		var startY:Number = insetsY - viewPosition.y + getTableHeader().getHeight();
...
//        hasFocus = (rowIsLead && colIsLead) && isFocusOwner();
	...
	private function synCellClasses():Void{
		...
		for(var i:Number = lastColumnCellFactories.length-1; i>=0; i--){
			if(lastColumnCellFactories[i] != this.getCellRenderer(0, i)){
				clearCells();
				return;
			}
		}
	...
	private function synCreateCellInstances():Void{
		...
		for(var i:Number=0; i<columnCount; i++){
			lastColumnCellFactories.push(getCellRenderer(0, i));
		}
		
		//create needed
		if(rowCells.length < needNum){
			var addNum:Number = needNum - rowCells.length;
			//trace("----create cells-----" + addNum);
	...
	private function addCellToContainer(cell:TableCell):Void{
		...
		append(cellCom);
		comToCellMap.put(cellCom.getID(), cell);
		//cellCom.addEventListener(itemHandler);
	}
    ...
    public function getExtentSize():Dimension{
    	var d:Dimension = getInsets().inroundsSize(getSize());
    ...
    public function getViewSize():Dimension{
    	var insets:Insets = getInsets();
    	var d:Dimension = insets.inroundsSize(getPreferredSize());
    	if(getAutoResizeMode() != AUTO_RESIZE_OFF){
    		d.width = getExtentSize().width;
    	}
    	else{
    		d.width = getColumnModel().getTotalColumnWidth();
    	}
    	d.height -= getTableHeader().getHeight();
    	return d;
    }
