...
class org.aswing.plaf.basic.BasicScrollBarUI extends ScrollBarUI
    ...
    private static var initialScrollSpeedThrottle:Number = 300; // first delay in milli seconds
    ...
    private function createArrowButton(direction:Number):JButton{
		var b:JButton = new JButton(null, createArrowIcon(direction));
		return b;
    }
	...
	private function installComponents():Void{
        if(isVertical()){
        	incrButton = createArrowButton(Math.PI/2);
        	decrButton = createArrowButton(-Math.PI/2);
        }else{
        	incrButton = createArrowButton(0);
        	decrButton = createArrowButton(-Math.PI);
        }
    ...
    private function paintAndLocateThumb(b:Rectangle):Void{
    	...
    	if(isVertical()){
    		...
    		thumbLength = trackLength*(extent/range);
    	}else{
    	...
    	if(thumbLength >= trackLength){
    		if(isVertical())
    			trace("Logic Wrong : Scrollbar exent="+extent+", range="+range);
    		thumMC._visible = false;
    		return;
    	}
    	
		var thumbRange:Number = trackLength - thumbLength;
		var thumbPos:Number = Math.round(thumbRange * ((value - min) / (range - extent)));
		if(isVertical()){
    ...
    private function setThumbRect(x:Number, y:Number, w:Number, h:Number):Void{
    	...
    	if(w != oldW || h != oldH){
    		paintThumb(thumMC, thumbRect.getSize());
    	}
    	thumMC._x = x;
    	thumMC._y = y;
    }
    ...
    public function preferredLayoutSize(target:Container):Dimension{
    	if(target == scrollbar){
    		...
    		if(isVertical()){
    			w = scrollBarWidth;
    			h = scrollBarWidth*2 + minimumThumbLength;
    		}else{
    			w = scrollBarWidth*2 + minimumThumbLength;
    			h = scrollBarWidth;
    		}
    		return scrollbar.getInsets().roundsSize(new Dimension(w, h));
    	}else{
    		trace("BasicScrollBarUI just can be JScrollBar's Layout : " + target);
