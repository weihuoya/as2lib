...
class org.aswing.plaf.basic.BasicTabbedPaneUI extends TabbedPaneUI {
    ...
    public function paintFocus(c:Component, g:Graphics):Void{
    	...
    	if(selectedIndex >= 0){
	    	...
	    	g.drawRectangle(new Pen(new ASColor(0x40FF40, 30), 1), x1 + 0.5, y1 + 0.5, (x2-x1)-1, (y2-y1)-1);
	    	g.drawRectangle(new Pen(new ASColor(0x40FF40, 50), 1), x1 + 1.5, y1 + 1.5, (x2-x1)-3, (y2-y1)-3);
    	}
    }
    ...
    private function drawTabAt(index:Number, bounds:Rectangle, paneBounds:Rectangle, g:Graphics, transformedTabMargin:Insets):Void{
		...
		tf.text = text;
    	ASWingUtils.applyTextFontAndColor(tf, tabbedPane.getFont(), tabbedPane.getForeground());
		tf._x = textRect.x;
		tf._y = textRect.y;
		tf._visible = true;
    }
    ...
    public function preferredLayoutSize(target:Container):Dimension{
    	...
    	prefferedSize = insets.roundsSize(new Dimension(w, h));
    	return prefferedSize;
    }
    ...
    public function minimumLayoutSize(target:Container):Dimension{
    	...
    	minimumSize = insets.roundsSize(new Dimension(w, h));
    	return minimumSize;
    }
