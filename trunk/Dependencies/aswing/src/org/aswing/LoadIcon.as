...
class org.aswing.LoadIcon extends ComponentDecorator implements Icon{
	...
	private function __onMCLoaded(target:MovieClip) : Void {
		if(!sizeInited){
			...
			var coms:Array = servicedComs.keys();
			for(var i:Number=0; i<coms.length; i++){
				var com:Component = Component(coms[i]);
				com.revalidate();
			}
			sizeInited = true;
		}
	...
	public function paintIcon(com:Component, g:Graphics, x:Number, y:Number):Void{
		...
		if(needLoad && MCUtils.isMovieClipExist(mc)){
			...
			servicedComs.put(com.getID(), com);
			//avoid flash8 loaded swf mask bug
			movieClipLoader.loadClip(url, creater.createMCWithName(imageMC, "inner"));
		}
	}
