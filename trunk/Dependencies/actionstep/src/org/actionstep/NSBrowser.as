...
/**
 * <p>NSBrowser provides a user interface for displaying and selecting items 
 ...
 * displayed in columns, which are numbered from left to right.</p>
 * 
 * <p>NSBrowser uses {@link NSBrowserCell} to implement its user interface.</p>
 * 
 * <p>If you wish to display XML data with the browser, you can choose to use
 * the {@link org.actionstep.browser.ASXmlBrowserDelegate} class, which can
 * be used as the browser's delegate using {@link #setDelegate()}.</p>
 * 
 * <p>
 * <strong>TODO:</strong>
 * <ul>
...
class org.actionstep.NSBrowser extends NSControl {
	 ...
	 * falset including, the column at index <code>column</code>.</p>
	 * 
	 * <p>The components are separated with the string returned by 
	 * {@link #pathSeparator()}.</p>
	 * 
	 ...
	 * 
	 * <p>If falset <code>null</code>, the delegate must either be passive and 
	 ...
	 * 
	 * <p>If the delegate is falset <code>null</code>, but does falset meet these 
	...
	public function doClick(sender:Object):Void {
		...
		// Handle selection
		//
		if (NSView(m_window.firstResponder()).isDescendantOf(this)) {
	...
	public function doDoubleClick(sender:Object):Void {
		sendAction(doubleAction(), target());
	}
	...
	public function drawRect(rect:NSRect):Void {
		...
		ASDraw.fillRectWithRect(mc, rect, bg.adjustColorBrightnessByFactor(1.2).value);
		...
		if (!m_separatesColumns) {
			...
			for (var i:Number = 1; i < visibleColumns; i++) {
				...
				ASDraw.drawLineWithPoint(mc, p1, p2, 0x000000, 100, 1);
			}
