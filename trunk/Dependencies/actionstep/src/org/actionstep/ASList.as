...
class org.actionstep.ASList extends NSControl {
  ...
  */
  ...
  private function computeValues() {
    ...
    m_maxScrollAmount = listCount - m_numberOfVisibleItems;
  ...
  private function resetTextFields() {
    if (m_mcBounds == null) {
      return;
    }
  ...
  private function createTextFields() {
    if (m_textFields == null) {
      ...
      for (var i:Number = 0;i<m_numberOfVisibleItems;i++) {
        ...
        tf.editable = false;
        tf.selectable = false;
        tf.embedFonts = m_font.isEmbedded();
  ...
  public function drawRect(rect:NSRect) {
    ...
    if (m_showsFirstResponder) {
      theme.drawFirstResponderWithRectInClip(rect, m_mcBounds);
