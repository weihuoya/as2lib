...
/**
 * The panel used by <code>NSAlert</code>s.
 *
 * Used internally.
 *
...
class org.actionstep.alert.ASAlertPanel extends NSPanel {
  ...
  //******************************************************
	...
	private function positionElements():Void {
		...
		rect.size.height = m_btns[i].frame().maxY() + BtnBottom + rootView().titleRect().size.height;
		setFrame(rect);

		//fill content with white
		ASDraw.fillRectWithRect(content.mcBounds(), content.bounds(), 0xFFFFFF);
	}
	...
	private function __modal(res:Object):Void {
	...
	private function setTitleMessageDefAltOther(title:String, message:String,
		...
		for(var i:Object in m_btns) {
			var btnTxt:String = arguments[parseInt(i)+2];
			if (btnTxt == null) {
				m_btns[i].removeFromSuperview();
			} else {
				m_btns[i].setTitle(btnTxt);
			}
		}
	...
	private static function getSomePanel(
		...
		panel.setTitleMessageDefAltOther(title, message, defaultButton,
			alternateButton, otherButton, sheet, info);
	...
	public static function NSBeginAlertSheet(title:String, defaultButton:String,
			alternateButton:String, otherButton:String, docWindow:NSWindow,
			modalDelegate:Object, didEndSelector:String, didDismissSelector:String,
			contextInfo:Object, msg:String, info:String):Void {
		__NSRunAlertSheet(__NSGetAlertSheet(title, defaultButton, alternateButton, otherButton, msg, info),
		docWindow, modalDelegate, didEndSelector, didDismissSelector, contextInfo);
	}

	public static function __NSGetAlertSheet
	(title:String, defaultButton:String, alternateButton:String, otherButton:String,
	msg:String, info:String):ASAlertPanel {
		// assume that all buttons are not null
	...
	public static function __NSRunAlertSheet
	(panel:ASAlertPanel, docWindow:NSWindow,
	modalDelegate:Object, didEndSelector:String, didDismissSelector:String,
	contextInfo:Object):Void {
