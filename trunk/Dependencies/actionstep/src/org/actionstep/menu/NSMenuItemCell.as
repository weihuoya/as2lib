...
/**
 * Handles the measurement and display of an instance of <code>NSMenuItem</code>.
 * It works in conjunction with an instance of <code>NSMenuView</code>, to
 * control the overall appearance of the menu.
 *
 * Please note that cell masks in <code>highlightsBy</code> and display-related
 * properties like <code>isBezeled</code> are ignored.
 *
 * The following methods override their NSCell counterparts, to prevent the
...
class org.actionstep.menu.NSMenuItemCell extends NSButtonCell{
	...
	public function imageRectForBounds(rect:NSRect):NSRect {
		...
		cellFrame.origin.x += !m_menuView.isHorizontal() ?
	...
	public function drawInteriorWithFrameInView(cellFrame:NSRect, controlView:NSView):Void {
		if(isHighlighted()) {
		  if (m_menuItem.isEnabled()) {
  			m_textColor = NSColor.selectedMenuItemTextColor();
  			m_backgroundColor = NSColor.selectedMenuItemColor();
  		} else {
  		  m_textColor = ASColors.lightGrayColor();
  			m_backgroundColor = null;
  		}
		} else {
			m_backgroundColor = null;
		  if (m_menuItem.isEnabled()) {
  			m_textColor = NSColor.textColor();
  		} else {
  		  m_textColor = ASColors.lightGrayColor();
  		}
		}
