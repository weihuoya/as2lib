...
class org.actionstep.debug.ASDebuggingPanel extends NSView {
	...
	private static function stageDidResize(ntf:NSNotification):Void
	{
		...
		g_instance.window().setFrame(
			new NSRect(0, stageSize.height - g_instance.window().frame().height(),
			stageSize.width, PANEL_CONTENT_HEIGHT));
	}
	...
	{
		var frame:NSRect = NSView(ntf.object).frame();

		if (frame.height() == 0)
		{
			g_instance.window().setFrameOrigin(
				g_instance.window().frame().origin.addPoint(
					new NSPoint(0, PANEL_CONTENT_HEIGHT)));
		} else {
			g_instance.window().setFrameOrigin(
				g_instance.window().frame().origin.subtractPoint(
					new NSPoint(0, PANEL_CONTENT_HEIGHT)));
		}
	}
}