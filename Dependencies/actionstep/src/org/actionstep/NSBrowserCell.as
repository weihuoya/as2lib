...
/**
 * <p>NSBrowserCell is the subclass of NSCell used by default to display data in 
 * the columns of an <code>NSBrowser</code>. (Each column contains an NSMatrix 
 * filled with NSBrowserCells.)</p>
 * 
 * <p>NSBrowserCell implements the user interface of 
 * {@link org.actionstep.NSBrowser}.</p>
 * 
 * <p>The branch, and highlighted branch images are provided by the current
 * {@link ASTheme}. Their names are <code>"NSBrowserBranch"</code> and
 * <code>"NSHighlightedBrowserBranch"</code> respectively.</p>
 * 
...
class org.actionstep.NSBrowserCell extends NSCell {
	...
	/**
	 * Returns the default image for branch NSBrowserCells (a right-pointing 
	 * triangle).
	 */
	public static function branchImage():NSImage {
		return NSImage.imageNamed("NSBrowserBranch");
	}
	
	/**
	 * Returns the default NSImage for branch NSBrowserCells that are 
	 * highlighted (a lighter version of the image returned by 
	 * {@link #branchImage}).
	 */
	public static function highlightedBranchImage():NSImage {
		return NSImage.imageNamed("NSHighlightedBrowserBranch");
	}
	...
	public function drawInteriorWithFrameInView(cellFrame:NSRect, 
		...
		if (isHighlighted() || state()) {
			...
			//
			if (!m_isLeaf) {
				branch_image = highlightedBranchImage();
			}
