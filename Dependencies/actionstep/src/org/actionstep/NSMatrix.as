...
//		return NSCell(m_cells.objectAtIndex(selLoc));
	...
	private function mouseTrackingCallbackListMode(mouseUp:Boolean):Void {		
	...
	private function mouseTrackingCallbackNonListMode(mouseUp:Boolean):Void {
		//trace("NSMatrix.mouseTrackingCallbackNonListMode(mouseUp)");
	...
	private function mouseDownListMode(theEvent:NSEvent):Void {		
		//trace("NSMatrix.mouseDownListMode(theEvent)");
	...
	private function mouseDownNonListMode(theEvent:NSEvent):Void {
		//trace("NSMatrix.mouseDownNonListMode(theEvent)");
		...
		if (loc != null) {
			...
			if (cell.isEnabled()) {
				...
				if(theEvent.view == this && cellTrackingRect().pointInRect(
					...
					cell.setTrackingCallbackSelector(this, "mouseTrackingCallbackNonListMode");
		...
		NSApplication.sharedApplication().callObjectSelectorWithNextEventMatchingMaskDequeue(this, 
			"mouseTrackingCallbackNonListMode", m_trackingData.eventMask, true);
	}
	...
	private function _rebuildLayoutAfterResizing():Void {
		if (m_autosizeCells) {
			...
			if (m_numRows > 1) {
				m_cellSize.height = m_bounds.size.height - ((m_numRows - 1) * m_cellSpacing.height);
				m_cellSize.height = m_cellSize.height / m_numRows;
				if (m_cellSize.height < 0) {
					m_cellSize.height = 0;
				}
			} else {
				m_cellSize.height = m_bounds.size.height;
			}
			
			if (m_numCols > 1) {
				m_cellSize.width = m_bounds.size.width - ((m_numCols - 1) * m_cellSpacing.width);
				m_cellSize.width = m_cellSize.width / m_numCols;
				if (m_cellSize.width < 0) {
					m_cellSize.width = 0;
				}
			} else {
				m_cellSize.width = m_bounds.size.width;
			}
		} else { // !autosizesCells
			// Keep the cell size as it is, and adjust the intercell to fit.
			if (m_numRows > 1) {
				m_cellSpacing.height = m_bounds.size.height - (m_numRows * m_cellSize.height);
			...
			if (m_numCols > 1) {
				m_cellSpacing.width = m_bounds.size.width - (m_numCols * m_cellSize.width);
