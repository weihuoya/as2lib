...
class org.actionstep.NSButton extends NSControl {
  ...
  public function setAlternateImage(image:NSImage) {
  	//
  	// Unregister this as observer of old image
  	//
    if (alternateImage() != null) {
      NSNotificationCenter.defaultCenter().removeObserverNameObject(
        this, NSImage.NSImageDidLoadNotification, alternateImage());
    }

    NSButtonCell(m_cell).setAlternateImage(image);

    //
    // Mark the button as needing redisplay is the image is loaded, or register
    // as an observer if it is not.
    //
    if (image == null || image.isRepresentationLoaded()) {
      setNeedsDisplay(true);
    } else {
      NSNotificationCenter.defaultCenter().addObserverSelectorNameObject(
        this, "imageDidLoad", NSImage.NSImageDidLoadNotification, image);
    }
  }
  ...
  public function setImage(anImage:NSImage) {
    ...
    if (image() != null) {
      NSNotificationCenter.defaultCenter().removeObserverNameObject(
        this, NSImage.NSImageDidLoadNotification, image());
    }
    ...
    if (image == null || image.isRepresentationLoaded()) {
      ...
      NSNotificationCenter.defaultCenter().addObserverSelectorNameObject(
        this, "imageDidLoad", NSImage.NSImageDidLoadNotification, anImage);
    }
  }
  ...
  /**
   * If an image has loaded, mark it as needing redisplay.
   */
