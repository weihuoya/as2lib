...
class org.actionstep.remoting.ASRecordSet extends NSObject{
	...
	/**
	 * Helper function to initialize the <code>m_colNames</code> dictionary.
	 */
	private function initColumns(colNames:Array):Void {
		...
		m_colNames = NSDictionary.dictionaryWithObjectsForKeys(
	...
	//*                 Member variables
	//******************************************************
	private var m_colNames:NSDictionary;
	private var m_items:NSDictionary;
	...
	public function ASRecordSet(colNames:Array) {
		m_items = NSDictionary.dictionary();
		m_itemId = "";
	...
	/**
	 * Be careful when invoking this method, since it will overwrite <code>m_colNames</code>,
	 * <code>m_items</code>, and other properties with the values found in the argument.
	 *
	...
	public function initWithResponseForService(data:Object, service:ASService):ASRecordSet {
		...
		m_service = ASService.serviceForNameURL(m_serviceName, service.connection().URL());
		if(m_service==null) {
			m_service = (new ASService()).
			initWithNameConnectionTracing(m_serviceName, service.connection(), false);
	...
	public function valueForFieldAtIndex(field:String, index:Number):Object {
		...
		if (m_items.objectForKey(index.toString())) {
			return m_items.objectForKey(index.toString())[m_colNames.objectForKey(field)];
		} else {
	...
	public function setValueForFieldAtIndex(field:String, index:Number, value:Object):Void {
		var val:Object = m_items.objectForKey(index.toString());
		val[m_colNames.objectForKey(field)] = value;
		m_items.setObjectForKey(val, index.toString());
	}
	...
	public function replaceRecordAtIndex(val:Array, index:Number):Void {
		if(val.length!=m_colNames.count()) {
			var e:NSException = NSException.exceptionWithNameReasonUserInfo(
	...
	public function valuesForField(field:String):Array {
		var ret:Array = [];
		var items:Array = m_items.allKeys().internalList();
	...
	public function insertRecord(val:Array):Void {
		try {
			insertRecordAtIndex(val, m_items.count());
		} catch (e:NSException) {
			trace(e);
			throw e;
		}
	...
	public function insertColumnAtIndex(col:String, index:Number):Void {
		try {
			...
			var i:Number = m_items.count();
			var arr:Array;
			while(i--) {
				arr = Array(m_items.objectForKey(i.toString()));
				arr.splice(index, 0, DEFAULT_PAD);
			}
			m_colNames.setObjectForKey(index, col);
		} catch (e:NSException) {
			trace(e);
			throw e;
		}
	...
	public function insertColumn(col:String):Void {
		try {
			insertColumnAtIndex(col, m_colNames.count());
		} catch (e:NSException) {
			trace(e);
			throw e;
		}
	...
	private function didReceiveResponse(res:ASResponse):Void {
		...
		while(i--) {
			index = i+j;
			m_items.setObjectForKey(items[i], index.toString());
		}
	...
	public function lastIndex():Number {
		return parseInt(m_items.allKeys().internalList()[0])+1;
	}
