...
import org.actionstep.remoting.ASRecordSet;
 ...
 */
class org.actionstep.remoting.ASPendingCall extends NSObject {
	...
	public function beginTimeoutWithSeconds(seconds:Number):Void {
		...
		//
		if (seconds == ASService.ASNoTimeOut) {
			return;
		}
	...
	private function onResult(result:Object):Void {
		...
		// Map the datatypes
		//
		result = mapDataTypes(result, service);
	...
	private function __mapDataTypes(result:Object, service:ASService):Void {
		...
		for(var i:String in result) {
			if(ASRecordSet.isRecognizedType(result[i])) {
				...
				initWithResponseForService(result[i], service));
			} else {
