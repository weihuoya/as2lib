...
class org.actionstep.asml.ASParsingUtils
{
	...
	public static function extractTypedValueFromString(string:String):Object
	{
		...
		//
		// Constant (only supports ored constants right now)
		//
		var ors:Array = string.split("|");

		//
		// Parse the constants
		//
		var len:Number = ors.length;
		for (var i:Number = 0; i < len; i++)
		{
			var cnst:String = ors[i];

			//! FIXME Something seems wrong about this.
			if (cnst.indexOf("org.actionstep.") != 0
				&& eval("org.actionstep." + cnst) != undefined)
			{
				cnst = "org.actionstep." + cnst;
			}
			else if (cnst.indexOf("org.actionstep.constants.") != 0
				&& eval("org.actionstep.constants." + cnst) != undefined)
			{
				cnst = "org.actionstep.constants." + cnst;
			}

			ors[i] = eval(cnst) == undefined ? 0 : eval(cnst);
		}

		//
		// Return just the constant if we're not oring anything.
		//
		if (ors.length == 1)
		{
			return ors[0];
		...
		for (var i:Number = 0; i < len; i++)
		{
			if (ors[i] instanceof ASConstantValue)
			{
				res |= ors[i].valueOf();
			}
			else
