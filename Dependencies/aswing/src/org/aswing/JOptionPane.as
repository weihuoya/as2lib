...
class org.aswing.JOptionPane extends JPanel {
	...
	private function addButton(button:JButton):Void{
	...
	private function addCloseListener(button:JButton):Void{
		...
		button.addActionListener(function(){ f.dispose(); });
	}
	...
	public static function showMessageDialog(title:String, msg:String, finishHandler:Function, parentComponent:Component, modal:Boolean, icon:Icon, buttons:Number):JOptionPane{
		var owner:JWindow = ASWingUtils.getWindowAncestor(parentComponent);
		if(modal == undefined) modal = true;
		var frame:JFrame = new JFrame(owner, title, modal);
		...
		frame.pack();
		frame.setLocation((Stage.width-frame.getWidth())/2, (Stage.height-frame.getHeight())/2);
		frame.show();
		return pane;
	}
