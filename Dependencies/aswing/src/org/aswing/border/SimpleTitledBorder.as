...
class org.aswing.border.SimpleTitledBorder extends DecorateBorder{
	...
	public function SimpleTitledBorder(interior:Border, title:String, position:Number, align:Number, offset:Number, font:ASFont, color:ASColor){
		...
		this.font = (font==undefined ? ASFont.getASFont(null , null) : font);
	...
	public function paintBorderImp(c:Component, g:Graphics, bounds:Rectangle):Void{
    	...
    	var width:Number = text_field._width;
    	var height:Number = text_field._height;
    	var x:Number = offset;
    	if(align == LEFT){
    		x += bounds.x;
    ...
    public function getBorderInsetsImp(c:Component, bounds:Rectangle):Insets{
		...
		if(cs.width < textFieldExtent.getTextFieldWidth()){
			var delta:Number = textFieldExtent.getTextFieldWidth() - cs.width;
			if(align == RIGHT){
				insets.left = delta;
    	...
    	if(position == BOTTOM){
    		insets.bottom = EDGE_SPACING*2 + textFieldExtent.getTextFieldHeight();
    	}else{
    		insets.top = EDGE_SPACING*2 + textFieldExtent.getTextFieldHeight();
    	}
    	return insets;
    }
