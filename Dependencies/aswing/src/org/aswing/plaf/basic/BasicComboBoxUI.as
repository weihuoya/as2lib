...
class org.aswing.plaf.basic.BasicComboBoxUI extends ComboBoxUI implements LayoutManager{
	...
	private function installDefaults():Void{
        ...
        box.setLayout(this);
        shadowColor = UIManager.getColor("ComboBox.shadow");
	}
    ...
    private function getMCPane():MCPanel{
    	if(mcPane == null){
    ...
    private function viewPopup():Void{
    	...
    	var mcPanel:MCPanel = getMCPane();
    	if(!mcPanel.contains(popupPane)){
    		mcPanel.append(popupPane);
			var paneMC:MovieClip = mcPanel.getPanelMC();
			...
			var popupPaneMC:MovieClip = MovieClip(paneMC.getInstanceAtDepth(paneMC.getNextHighestDepth()-1));
    ...
    private function startMoveToView(height:Number):Point{
    	...
    	var visibleBounds:Rectangle = ASWingUtils.getVisibleMaximizedBounds(getMCPane().getPanelMC());
    ...
    private function returnFocusFromListToCombobox():Void{
    	...
    	if(editorCom.isEnabled()){
    		if(editorCom.requestFocus()){
    			return;
    		}
    	}
		if(!dropDownButton.requestFocus()){
			box.transferFocus();
		}
    }
	...
	public function isPopupVisible(c:JComboBox):Boolean{
		return getScollPane().isDisplayable() && getScollPane().isVisible();
	}
    ...
    public function preferredLayoutSize(target:Container):Dimension{
    	...
    	return insets.roundsSize(buttonSize);
    }

    public function minimumLayoutSize(target:Container):Dimension{
    	return box.getInsets().roundsSize(dropDownButton.getPreferredSize());
    }
