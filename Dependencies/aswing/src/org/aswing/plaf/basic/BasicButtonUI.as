...
class org.aswing.plaf.basic.BasicButtonUI extends ButtonUI{
	...
	private function installDefaults(b:AbstractButton):Void{
        ...
        if(!defaults_initialized) {
            defaultTextShiftOffset = Number(UIManager.get(pp + "textShiftOffset"));
    ...
    private function paintText(b:AbstractButton, textRect:Rectangle, text:String):Void{
    	...
    	if(model.isEnabled()){
    		paintTextField(textRect, getTopTextField(b), text, font, b.getForeground());
    		getBottomTextField(b)._visible = false;
    	}else{
    		getBottomTextField(b)._visible = true;
    		paintTextField(textRect, getTopTextField(b), text, font, b.getBackground().darker());
    		var rbRectangle:Rectangle = new Rectangle(textRect);
    		rbRectangle.x++;
    		rbRectangle.y++;
    		paintTextField(rbRectangle, getBottomTextField(b), text, font, b.getBackground().brighter());
    	}
    }
    
    private function paintTextField(tRect:Rectangle, textField:TextField, text:String, font:ASFont, color:ASColor):Void{
		textField.text = text;
    	ASWingUtils.applyTextFontAndColor(textField, font, color);
		//ASWingUtils.boundsTextField(textField, tRect); //This method has some problem cause text shake
    ...
    private function getButtonPreferredSize(b:AbstractButton, icon:Icon, text:String):Dimension{
        ...
        size = b.getInsets().roundsSize(size);
		if(b.getMargin() != null)
        	size = b.getMargin().roundsSize(size);
        return size;
    }
    /**
    ...
    private function getButtonMinimumSize(b:AbstractButton, icon:Icon, text:String):Dimension{
        var size:Dimension = b.getInsets().roundsSize();
		if(b.getMargin() != null)
        	size = b.getMargin().roundsSize(size);
		return size;
    }    
