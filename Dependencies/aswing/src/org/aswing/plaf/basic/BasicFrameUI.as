...
class org.aswing.plaf.basic.BasicFrameUI extends BasicWindowUI {
    ...
    private function installComponents():Void {
    	titleBar = new FrameTitleBar(frame);
	...
	private function installListeners():Void{
		...
		frame.addEventListener(JFrame.ON_WINDOW_ACTIVATED, frame.repaint, frame);
		frame.addEventListener(JFrame.ON_WINDOW_DEACTIVATED, frame.repaint, frame);
	}
    ...
    public function create(c:Component):Void{
    	super.create(c);
    	boundsMC = frame.createDragRepresentMC();
    }
    ...
    private function __onTitleBarRelease():Void{
    	...
    	if(frame.isDragable() && !isMaximizedFrame()){
	    	...
	    	frame.setLocation(loc.move(delta.x, delta.y));
	    	frame.revalidate();
    	}
    }
