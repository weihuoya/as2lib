...
class org.aswing.ASWingUtils{
     ...
     * @return the first ancestor of c that's ordinaryly a MCPanel or a JWindow(or JFrame).
     */
    ...
    public static function applyTextFormatAndColor(text:TextField, textFormat:ASTextFormat, color:ASColor):Void{
        textFormat.applyToText(text);
        textFormat.applyToTextForNew(text);
        ...
        textIconGap:Number):String
    {
        ...
        if(textIsEmpty){
            ...
            textR.width = ts.getTextFieldWidth();
            textR.height = ts.getTextFieldHeight();
        }
    ...
    public static function layoutTextWidth(text:String, textR:Rectangle, availTextWidth:Number, tf:ASTextFormat):String{
        ...
        var totalWidth:Number = computeStringWidth(tf, clipString);
        if(totalWidth > availTextWidth){
            totalWidth = computeStringWidth(tf, "..");
            if(totalWidth > availTextWidth){
                text = ".";
                textR.width = computeStringWidth(tf, ".");
                if(textR.width > availTextWidth){
                    textR.width = 0;
                    text = "";
                }
            ...
            while(li<ri){
                ...
                var length:Number = lastWidth + computeStringWidth(tf, subText);
                
                if((li == i - 1) && li>0){
                    if(length > availTextWidth){
                        subText = text.substring(0, li);
                        textR.width = lastWidth + computeStringWidth(tf, text.substring(0, li));
                    }else{
                        textR.width = length;
                    }
