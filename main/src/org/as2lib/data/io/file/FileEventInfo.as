/*
 * Copyright the original author or authors.
 * 
 * Licensed under the MOZILLA PUBLIC LICENSE, Version 1.1 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 * 
 *      http://www.mozilla.org/MPL/MPL-1.1.html
 * 
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

import org.as2lib.env.event.SimpleEventInfo;
import org.as2lib.data.io.file.File;

/**
 * Usual Informations to be published if a file event was called
 * 
 * @see File#addListener
 * @see org.as2lib.data.io.file.FileListener
 */
class org.as2lib.data.io.file.FileEventInfo extends SimpleEventInfo {
	
	/** Internal holder for the file that published the event */
	private var file:File;
	
	/**
	 * Constructs a new file event information
	 * 
	 * @param event Event to be published
	 * @param file File where the event was published
	 */
	public function FileEventInfo(event:String, file:File) {
		super(event);
		this.file = file;
	}
	
	/**
	 * Getter for the file that published the event.
	 * 
	 * @return file that was published with the event.
	 */
	public function getFile(Void):File {
		return file;
	}
}